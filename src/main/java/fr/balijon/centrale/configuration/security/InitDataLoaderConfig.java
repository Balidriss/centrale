...
import fr.balijon.centrale.entity.*;
import fr.balijon.centrale.repository.*;
        ...
        if (addressRepository.count() == 0) {
            ...
            for (long i = 1L; i <= 200L; i++) {
                ...
                if (random.nextBoolean()) {
                    User user;
                    do {
                        user = users.get(random.nextInt(1, 200));
    ...
    private void createUsers() {
        if (userRepository.count() != 200) {
            for (long i = 1L; i <= 200L; i++) {
                ...
                Role role = roleRepository.findOneByLabel("ROLE_USER");
                if (i == 1L) {
                    role = roleRepository.findOneByLabel("ROLE_ADMIN");
                }
                roles.add(role);
                user.setRoles(roles);
                user.setPhoto("photoProfile");
    ...
    private void createListings() {
        ...
        if (listingRepository.count() < 1000) {
            for (int i = 1; i <= 1000; i++) {
                ...
                Address address = addressRepository.findById(random.nextLong(1L, 200L)).get();
                listing.setAddress(address);

                listing.setDescription(faker.lorem().sentence(45, 9));

                User user = users.get(random.nextInt(1, 200));
                ;
    ...
    private void createImages() {
        ...
        for (Listing listing : listings) {
            if (listing.getImages().isEmpty()) {
                ...
                image.setPath("voiture" + random.nextInt(1, 5));
        ...
        for (User user : users) {
            for (int i = 0; i < random.nextInt(2, 8); i++) {
                ...
                favorite.setCreatedAt(generateRandomDate(
                        Instant.now().minusSeconds(999999999)));
                UserListingId favoriteId = new UserListingId();
                favoriteId.setListingUuid(listing.getUuid());
                favoriteId.setUserUuid(user.getUuid());
